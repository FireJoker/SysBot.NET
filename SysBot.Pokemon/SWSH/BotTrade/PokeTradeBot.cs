...
namespace SysBot.Pokemon
{
    public class PokeTradeBot : PokeRoutineExecutor8, ICountBot
    {
        ...
        private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8SWSH sav, PokeTradeDetail<PK8> poke, CancellationToken token)
        {
            ...
            Log($"Found Link Trade partner: {trainerName}-{trainerTID[0]}-{trainerTID[1]} (NID: {trainerNID})");
            poke.SendNotification(this, $"Found Link Trade partner: {trainerName} TID: {trainerTID[0]} SID: {trainerTID[1]}. Waiting for a Pokémon...");
        ...
        private void UpdateCountsAndExport(PokeTradeDetail<PK8> poke, PK8 received, PK8 toSend)
        {
            ...
            if (DumpSetting.Dump && !string.IsNullOrEmpty(DumpSetting.DumpFolder))
            {
                ...
                if (poke.Type is PokeTradeType.Specific or PokeTradeType.Clone or PokeTradeType.FixOT or PokeTradeType.SupportTrade or PokeTradeType.TradeCord or PokeTradeType.Giveaway) DumpPokemon(DumpSetting.DumpFolder, "traded", toSend); // sent to partner
            }
        }
...
#pragma warning restore 4014
        ...
        private async Task<bool> WaitForPokemonChanged(uint offset, int waitms, int waitInterval, CancellationToken token)
        {
            ...
            return Hub.Config.Trade.SpinTrade ? 
                await SpinTrade(offset, oldEC, waitms, waitInterval, false, token).ConfigureAwait(false) : 
                await ReadUntilChanged(offset, oldEC, waitms, waitInterval, false, token).ConfigureAwait(false);
        }
